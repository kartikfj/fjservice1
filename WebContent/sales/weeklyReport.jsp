    ...
    <!-- Main content -->	    
        ...
        <div class="col-lg-4 col-md-4 col-sm-6 col-xs-12">
        	 ...
        	 <div class="form-group form-inline"> 
                <select class="form-control select2" id="slctDvsns" multiple="multiple" data-placeholder="Select Divisions" style="width:70%;"> </select>
            ...
            <div class="table-scroll"  id="table-scroll"  >
             <div class="table-wrap">
              <table class="main-table small" id="report-table">
                <thead>
                  <tr>
                  <th rowspan="3" width="65"   class="spclHeader" scope="col">Company</th> 
                  <th rowspan="3"  width="90"   class="spclHeader" scope="col">Division</th> 
                  <th rowspan="3"   class="spclHeader" scope="col">STAGES</th> 
                  <th colspan="4"  class="spclHeader" scope="col">Weekly Avg</th>                   
...
var uniqueDivisions = [...new Set(reportsList.map(item => item.division))];   

var filterOptions = [...new Set(reportsList.map(item => { return {'name' : item.division  ,'value' : item.division }  } )  )];   
...
uniqueDivisions.filter( item =>{
	     ...
	     companies =  [...new Set(reportsList.map(items => {
			if(items.company && items.division == item ) {
				return items.company;
			}
		}  ))];
	     companies.filter( divisionItem => {
			 if(!divisionItem) {
				 return;
			 }
			 //console.log("companies"+companies);
			 var obj={'division':item}; 
			 for(var i=${currWeek},s2CntSum=0,s3CntSum=0,s4CntSum=0,s5CntSum=0,s2ValSum=0,s3ValSum=0,s4ValSum=0,s5ValSum=0; i>= 1 ; i --){
					...
					var filteredValues = reportsList.find( singleRw =>  singleRw.division == item &&  singleRw.company == divisionItem  && singleRw.week == weekTmp  );
					 ...
					 if(typeof filteredValues === 'undefined'){
			    		 ...
			    		 company = filteredValues.company;			    				    	
			    		 ...
			    		 //console.log("filteredValues== "+filteredValues.company+""+filteredValues.division+""+divisionItem);
			    		 if(divisionItem == filteredValues.company){			    			
...
function createReport(){
	...
	// console.dir(extractedReport);
	 ...
	 output += "<tr><td rowspan='2' class='txt-left spclHeader' scope='col'>"+item.company+"</td><td rowspan='2' class='txt-left spclHeader' scope='col'>"+item.division+"</td><td class='txt-left spclHeader' scope='col'>Count</td> ";

	 output += "<td class='txt-right spclHeader' scope='col' >"+Math.round(item.s2CntSum/weeknumber)+"</td>";
	 output += "<td class='txt-right spclHeader' scope='col' >"+Math.round(item.s3CntSum/weeknumber)+"</td>";
	 output += "<td class='txt-right spclHeader' scope='col' >"+Math.round(item.s4CntSum/weeknumber)+"</td>";
	 output += "<td class='txt-right spclHeader' scope='col' >"+Math.round(item.s5CntSum/weeknumber)+"</td>";	
	  ...
	  output += "<td class='txt-right spclHeader' scope='col' >"+Math.round(item.s2ValSum/weeknumber)+"</td>";
	  output += "<td class='txt-right spclHeader' scope='col' >"+Math.round(item.s3ValSum/weeknumber)+"</td>";
	  output += "<td class='txt-right spclHeader' scope='col' >"+Math.round(item.s4ValSum/weeknumber)+"</td>";
	  output += "<td class='txt-right spclHeader' scope='col' >"+Math.round(item.s5ValSum/weeknumber)+"</td>";
	  output += valueRws; 
	  output += "</tr>";
	  countRws = '';
	  valueRws = '';
...
function downloadReports(weeknumber, extractedReport){ 
	...
	exportTable +="<tr><th>Company</th><th>Division</th><th></th><th>Weekly Avg/S2</th><th>Weekly Avg/S3</th><th>Weekly Avg/S4</th><th>Weekly Avg/S5</th>";
	 ...
	 extractedReport.map(item => { 
		 exportTable += "<tr><td>"+item.company+"</td><td>"+item.division+"</td><td>Count</td><td>"+Math.round(item.s2CntSum/weeknumber)+"</td><td>"+Math.round(item.s3CntSum/weeknumber)+"</td><td>"+Math.round(item.s4CntSum/weeknumber)+"</td><td>"+Math.round(item.s5CntSum/weeknumber)+"</td>";		
		  for(var i=${currWeek};i>=1;i--){ 
		  ...
		  exportTable += "</tr><tr><td>"+item.company+"</td><td>"+item.division+"</td><td>Value</td><td>"+Math.round(item.s2ValSum/weeknumber)+"</td><td>"+Math.round(item.s3ValSum/weeknumber)+"</td><td>"+Math.round(item.s4ValSum/weeknumber)+"</td><td>"+Math.round(item.s5ValSum/weeknumber)+"</td> ";
	 ...
	 exportTable += "</tbody></table>";	 
	  ...
	  table = $('#exclexprtTble').DataTable({   dom: 'Bfrtip', "paging":   false,  
	   ...
	   buttons: [{ extend: 'excelHtml5', text:      '<i class="fa fa-file-excel-o" style="color: #fff; font-size: 1.5em;">Download</i>', filename: 'WEEK WISE SALES REPORT - ${currCal}',
...
function customeReportView(){
	...
	if(uniqueDivisions === null || typeof uniqueDivisions === 'undefined' || uniqueDivisions === '' || uniqueDivisions.length == 0){
		 ...
		 totS2AvgCount = 0, totS2AvgVal = 0, totS3AvgCount = 0, totS3AvgVal = 0, totS4AvgCount = 0, totS4AvgVal = 0, totS5AvgCount = 0, totS5AvgVal = 0;
		 exportTable = "<table class='table' id='exclexprtTble'><thead>";
	     $("#exclData").html(""); 
		 $("#wrTBody").html("<tr><td>Loading......</td></tr>");  
		 //alert("uniqueDivisions=="+uniqueDivisions);
		 createCustomeReportObj(uniqueDivisions);
		 createReport();
	}
