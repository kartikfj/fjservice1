...
.container { padding-right: 0px !important;padding-left: 0px !important; width:80%}
		 ...
		 success: function(data) { 
			 ...
			 if(parseInt(details.stage) === 4){
				 output += '<table id="stage_detail_tbl4" class="table table-bordered table-hover small">'+
					'<thead>'+       		
		 			'<tr>	'+	        			 
		 			'<th>Sales Eng.</th><th>SO Dt.</th><th>SO Code-No.</th><th>Project</th><th>Balance Value</th><th>Exp.Billing Date</th><th>Short Close</th><th>Reminders</th>'+	
		 			'<th>Items</th>'+      
		 			'</tr></thead><tbody>'; 
				 ...
				 if(data){
					...
					const showUpdateBtn = (getSalesEngineerDetails(data[i].seCode)[0].salesman_emp_code === loggedEmp) ? true : false; 
					 ...
					 output +=  ' </td>';					 
			 		 ...
			 		 if(showUpdateBtn){ 
			 		 	output += '<label style="color:blue;text-transform: uppercase;"><input type="radio" name="invupdate" id="inv'+$.trim(data[i].cqhSysId)+'" onClick=openLostHoldStg3Window(event,\''+$.trim(data[i].cqhSysId)+'\',\''+$.trim(data[i].qtnCode)+'\',\''+$.trim(data[i].qtnNo)+'\',\'INV\',\''+$.trim(data[i].qtnDt.substring(0, 10)).split("-").reverse().join("/")+'\',\''+$.trim(data[i].seCode)+'\')> Update'	 		 
			 		  			 '</label>';
			 		   }
			 		  output +=  '</td> ';
			 		 output += ' <td>';
					 ...
					 output += '</td>';
			 		 if(showUpdateBtn){   
						 output += '<td><button class="btn btn-xs btn-danger" id="'+$.trim(data[i].cqhSysId)+'" onclick=openRequestWindow({seCode:\''+$.trim(data[i].seCode)+'\',row:'+i+',id:'+$.trim(data[i].cqhSysId)+',event})>followup</button></td>';
					 }else{
				 ...
				 if(data){
					...
					const showUpdateBtn = (getSalesEngineerDetails(data[i].seCode)[0].salesman_emp_code === loggedEmp) ? true : false; 
					 ...
					 output +=  ' <td>';
					// if(showUpdateBtn){   
						 output += ' <button class="btn btn-xs btn-success"  name="reminders" id="R'+$.trim(data[i].cqhSysId)+'" onClick=openLostHoldStg3Window(event,\''+$.trim(data[i].cqhSysId)+'\',\''+$.trim(data[i].qtnCode)+'\',\''+$.trim(data[i].qtnNo)+'\',\'R\',\''+$.trim(data[i].qtnDt.substring(0, 10)).split("-").reverse().join("/")+'\',\''+$.trim(data[i].seCode)+'\',\''+encodeURIComponent($.trim(data[i].projectName))+'\',\''+$.trim(data[i].qtnCode)+-+$.trim(data[i].qtnNo)+'\')> Reminder </button>' 
						 output += ' <div id="innerday'+$.trim(data[i].cqhSysId)+'" class="cen_icon" onclick="showRemindersForQtn(event,\''+$.trim(data[i].qtnCode)+-+$.trim(data[i].qtnNo)+'\');"> '     	
						 if($.trim(data[i].reminderCount) > 0){ 
							 output += '  <i class="fa fa-2x fa-clock-o"></i>';
						    }
				 ...
				 output += '<table id="stage1_detail_tbl" class="table table-bordered table-hover small">'+
					'<thead>'+       		
		 			'<tr>	'+	        			 
		 			'<th>Sales Eng.</th><th>Qtn Dt.</th><th>Qtn<br/> Code-No</th><th>Customer</th><th>Project</th><th>Consultant</th><th>Amount</th><th>Status</th><th>Priority</th><th>Remarks</th>'+	
		 			'<th  width="40px">Followup</th><th width="120px">Update Stage</th><th>Reminder</th>'+      
		 			'</tr></thead><tbody>'; 
				 if(data){
					 stage23List = data; 
					 var totalAmount = 0;remindersCount = 1;
	...
	if ((confirm('Are You sure, You Want to update this details!'))) {
		...
		$.ajax({
				 ...
				 if(parseInt(data)=== 1){ 
					 ...
					 //table.cell({row:details.row, column:7}).data(details.status)
					 //table.cell({row:details.row, column:8}).data(details.priority)
					// table.cell({row:details.row, column:9}).data(details.remarks)
...
function Apply(action){
	...
	if(data.status && data.remarks){ updateStage(data);}else{
...
function openLostHoldStg3Window(event,  id, qtnCode, qtnNo, type, qtanDate,segSalesCode,projectname,qtncodeNo){
